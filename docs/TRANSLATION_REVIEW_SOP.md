...
- 验证报告必须证明每个 `subtitle_zh` 可按稳定 ID 逐字重建、章节可见正文无增删、非章节末的句中换段为 0。只改换行且正式字幕/TTS 文本不变属于纯展示层修改，可以不重开 T/A/B，但必须生成递增阅读稿并重新取得哈希绑定批准；
...
验证报告必须记录正式翻译、章节来源、时轴映射和阅读稿的路径及 SHA-256，并证明章节数、槽数、分配数、缺失数、重复数和文本一致性。用户批准记录必须同时绑定阅读稿与正式翻译的路径及 SHA-256。任一输入变化时重新生成阅读稿并重新取得批准。
...
- 用户批准记录同时绑定章节阅读稿与正式翻译的路径及 SHA-256。
...
状态不是 `pass`、角色不独立、章节阅读稿未通过、用户批准未绑定哈希、文件缺失或哈希与生产输入不一致时，角色识别、选音、付费 TTS 和渲染全部锁定。
...
5. 重新生成按章节中文阅读稿与验证报告；
6. 重新取得绑定新阅读稿和新正式翻译哈希的用户批准，并生成新的 `translation_gate.json`；
