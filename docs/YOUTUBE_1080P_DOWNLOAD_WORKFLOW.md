...
每条视频的最高分辨率、编码、音轨语言和格式编号都可能不同，因此必须逐条探测，但不再等待人工确认。媒体 Agent 自动检查：
...
格式探测通过后，Agent 按“源语言与原声标记 → 真实分辨率/项目目标 → 编码与容器兼容性 → 帧率 → 码率”排序，写入 `qa/media_format_selection_vN.json` 并直接开始下载，不得要求用户回复类似“确认 137+140”的格式组合。多音轨元数据冲突时，先自动下载短探针并做语言验证；仍无法确认时将机器门禁标记为失败并报告“待修复”。
...
应用上述规则后立即生成 `qa/media_format_selection_vN.json`。至少记录完整格式探测文件及 SHA-256、候选排序、选中画面/音频编号、源语言证据、选择理由、`manual_approval_required=false` 和 `status=pass`；状态为 `pass` 后直接执行下载，不再设置格式批准文件或聊天确认。
...
如果目标视频提供 1440p/4K，但只有 VP9 或 AV1，自动保留最高画质下载母版，并用 `ffprobe` 与短片段解码验证剪辑兼容性；不能直接使用时另建 H.264 工作副本。不要为了容器扩展名是 MP4 就假定内部编码一定是 H.264。
...
不要只看格式编号。自动检查 `-F` 输出中的语言、`original (default)` 和编码；元数据冲突时用短探针语言检测。本次视频同时返回英语和西班牙语音轨，规则自动选择西班牙语 `140-1`。
...
- [ ] 自动验证视频编码和剪映兼容性
- [ ] 自动验证原声音轨语言
- [ ] `qa/media_format_selection_vN.json` 为 `pass`，且未请求人工格式批准
- [ ] 分别下载画面和音频
