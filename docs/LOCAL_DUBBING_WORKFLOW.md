...
这是一套由声轨工坊执行、并以门禁文件和 SHA-256 绑定生产证据的本地流程。输入是一个拥有下载和二创权限的视频链接或本地视频，输出是：
...
- 人工关卡只保留翻译通读、音色和最终观看等真正需要主观判断的环节；阅读稿展示后的明确选音指令自动批准当前翻译版本，广告处理、试听费用和“生成全片”后的全文付费调用不再额外等待批准；
- TTS 和视频渲染可从缓存续跑。
...
1. 总工作流（本文件）：`dub_workbench/docs/LOCAL_DUBBING_WORKFLOW.md`
2. 机器可读状态模型：`dub_workbench/docs/workflow.definition.json`
3. 高清下载 SOP：`YOUTUBE_1080P_DOWNLOAD_WORKFLOW.md`
4. 广告检测、剪除与遮盖 SOP：`dub_workbench/docs/AD_DETECTION_AND_OVERLAY_SOP.md`
5. 通用双 Agent 翻译审核 SOP：`dub_workbench/docs/TRANSLATION_REVIEW_SOP.md`
6. 工作台核心执行器：`dub_workbench/backend/workbench/`
7. 当前项目约束：`<video_id>_run/PROJECT.md`
8. 当前项目正式翻译稿、术语表、音色交接、QA 门禁和执行报告
...
| 翻译放行 | 阅读按章节排版的全文中文稿；如需修改直接提出，否则发出“开始选音色/启动工作台”等明确下游指令 | Agent T 直接全文翻译，两个审核 Agent 全文独立检查，主控合并、终检并生成章节阅读稿；收到下游指令后自行绑定当前哈希并生成批准记录，不再要求用户复述 SHA |
...
Cookie 与 API Key 都是敏感凭证：不粘贴到聊天、不写进文档、不进入 Git、不出现在命令输出。工作台通过操作系统密钥链保存 Provider 凭证；若显式使用临时内存后端，服务重启后必须重新输入。不得把 Key 文件放入代码仓库或项目迁移包。
...
```text
C:\Users\<用户名>\Downloads\www.youtube.com_cookies.txt
```
...
1. 链接下载会从探测结果中按冻结源语言优先选择人工字幕、再选择自动字幕；同一下载任务把 SRT/VTT 移入项目相对路径并生成 `work/frozen_source_vN.json` 与文本投影。探测报告只保存语言、来源和格式，不保存 yt-dlp 返回的临时字幕 URL；没有明确字幕时返回“需导入/设备 ASR”，不得宣称已经转写；
2. 在项目操作面板提交源字幕相对路径；后端通过 `POST /api/workflows/runs/{run_id}/ingest/source-transcript` 导入 SRT、VTT 或结构化 JSON，生成带稳定 ID 的 `work/frozen_source_vN.json`；
3. 源视频含字幕轨时，通过 `POST /api/workflows/runs/{run_id}/ingest/embedded-subtitle` 创建持久化 `media.extract_embedded_subtitle` 任务。Worker 实际执行受项目目录约束、禁止覆盖的 FFmpeg 命令，同时冻结字幕、稳定 ID JSON、纯文本投影和输入/输出哈希；旧的 `embedded-subtitle-plan` 端点仅用于查看安全计划；
4. 没有可验证字幕时，由当前设备绑定的 ASR 工具生成源语言 SRT/VTT/JSON。模型权重和 GPU 环境属于设备绑定，不写入 Git 或迁移包；工作台不会静默安装或调用 ASR 模型；
5. 广告分析提交完整候选、证据和 `remove/mask/keep` 决定。`POST /api/workflows/runs/{run_id}/ad-edit` 创建持久化任务：有删除段时同步裁切画面和原声，无删除段时也用 FFmpeg 安全 remux 成独立工作母版，随后全片探测/解码、生成 source→working 映射并调用现有 ad gate。遮盖计划只保留到最终渲染层；证据不完整时任务失败关闭，不会伪造 `no_ads_detected`；
6. `POST /api/workflows/runs/{run_id}/cover-source` 只允许从 `ad_edit_gate` 哈希绑定的正式工作母版按时间点抽取版本化 PNG。该制品供发布封面生成器使用，但不自动把授权、人物身份、文字或广告残留核对标为通过。
...
严格执行 `dub_workbench/docs/TRANSLATION_REVIEW_SOP.md`：
...
9. 暂停并把章节阅读稿交给用户整体阅读；用户如需修改可直接提出，若说“开始选音色”“启动工作台”“进入选音”或等价明确下游指令，则自动批准当前展示版本；
10. 主控自行计算并绑定阅读稿与正式翻译的 SHA-256，立即生成 `qa/translation_approval_vN.json` 和 `qa/translation_gate.json`，满足下面全部条件后直接执行用户要求的下游动作，不得再索要批准措辞或哈希。
...
7. 只改变阅读稿换行且正式字幕/TTS 文本不变时，属于纯展示层修订，可以不重开 T/A/B；仍须生成递增版本阅读稿和验证报告并使旧批准失效；用户针对新展示版本发出的明确下游执行指令可自动生成新的哈希绑定批准。
...
批准记录至少包含阅读稿路径与 SHA-256、正式翻译路径与 SHA-256、用户原始下游指令、批准时间和 `capture_mode=explicit_downstream_command`。主控必须自己计算实际哈希，不得让用户复制。阅读稿或正式翻译任一哈希变化时，旧批准和旧 `translation_gate` 立即失效。
...
阶段 07 由工作台 API 和本地任务执行器持有，不再依赖项目目录里的单次脚本。所有请求路径必须是项目根目录内的相对路径；前端不让用户手工填写音频时长、画面尺寸、重定时段或遮盖区间等可由正式制品推导的字段。

#### 7.1 音色锁与 TTS 输入

`POST /api/workflows/runs/{run_id}/voice-lock` 验证 `translation_gate`，将每个 `role_id` 唯一绑定到音色，并生成版本化音色锁与 `work/tts_segments_vN.json`。显示字幕始终来自 `subtitle_zh`；`recording_zh/tts_text` 只进入录音输入。全部片段固定 `speed=1.0`。

一分钟试听通过 `POST /api/workflows/runs/{run_id}/audition` 提交给本地任务执行器；请求只包含版本号，后端从同版本音色锁和自动生成的 `work/tts_segments_vN.json` 推导全部输入。音色锁定即构成试听授权，不再追加费用确认。后端按角色全覆盖、约 60 秒和固定字符上限确定性选段，所有请求保持 `speed=1.0`，最后用 FFmpeg 组装为 48 kHz 单声道 WAV 并做完整解码检查。试听选择、授权、逐段 TTS manifest、WAV 和机器检查清单均版本化且禁止覆盖；试听制品绑定当前翻译门禁、音色锁和冻结语音 Provider，授权范围不能作为全文授权。可能已计费但结果不确定时任务进入 `blocked_uncertain`，不得自动重发。

#### 7.2 “生成全片”指令、dry-run 与正式 TTS

`POST /api/workflows/runs/{run_id}/generate-full` 只接受“生成全片”“生成全文”等明确指令。执行前验证：
...
TTS 段文件与音色锁、正式翻译完全匹配
本次使用新的或与上述输入哈希完全匹配的输出目录
```

验证失败时不得通过“用户说继续”推断为放弃门禁；必须修复记录或回到上游阶段。

后端先把批次数、角色、字符数、费用估算和缓存复用写入 `qa/full_tts_authorization_vN.json`，然后在同一任务中继续生成，不再暂停等待二次确认。如冻结翻译、角色、音色、Provider 或 TTS 参数变化，必须创建新运行或新版本并重新取得明确指令。

执行器行为：

- 使用当前运行冻结的语音 Provider 与模型，不写死厂商；
- 中文速度固定 `1.0`；
- 逐片段原子写入音频和 `tts/manifest.json`；
- 哈希与冻结输入完全一致的 `ready` 片段自动复用；
...
#### 7.3 自动组装中文时间轴与视频重定时计划

正式 TTS 清单完成后，`POST /api/workflows/runs/{run_id}/render` 从哈希绑定的片段自动生成统一 48 kHz mono PCM WAV、中文字幕 SRT、中文时间轴和完整 `video_retime_plan_vN.json`。原对白间隙以静音保留；广告删除区间按 `source_to_edit_timeline` 映射，跨越被删除区间的槽位直接失败。
...
自动计划完成后、FFmpeg 正式渲染前，执行器生成 `qa/production_gate.json`。只有以下检查全部通过，状态才能为 `pass`：
...
渲染任务从工作母版、广告门禁、翻译门禁、音色锁、全文授权和 TTS manifest 推导全部媒体参数。默认使用可移植的 `libx264`；明确配置并通过诊断后可以选择设备可用的编码器。输出存在时拒绝覆盖，必须增加版本号。

完成标志：输出目录中存在可播放的最终 MP4、中文音轨、字幕、时间轴和重定时计划。

#### 7.6 成片后修正少量说话人

说话人或音色发生变化时，生成新的翻译/音色锁/TTS 输入版本和新的输出版本，绝不覆盖旧成片。只有内容哈希、角色、音色、模型和速度均未变化的 `ready` 片段可以复用；受影响片段重新 dry-run 和生成，再自动重建整轨、重定时计划、生产门禁与机器 QA。供应商返回完成状态不确定时按请求账本中的 trace 人工查账，禁止盲目重发。
...
```powershell
$final = Resolve-Path "<video_id>_run\deliverables\<final-video>.mp4"
...
- Agent T 全文直译以及 A/B 全文审核会消耗所选大模型 Provider 的额度。质量优先模式以翻译质量为第一目标；额度不足时保存进度等待恢复，不静默降低模型或切换其他 Provider。
...
| 分辨率 | 1920×1080 |
| 中文字幕 | 1,306 条 |
| MiniMax TTS | 157 批，58,342 字符 |
| 角色 | 主持人 A 69 句；主持人 B 151 句；嘉宾 1,086 句 |
| 画面区块 | 437 个 |
| 速度范围 | 0.590509–1.849689 倍 |
| 原声音量 | 8% |
| 从首个 TTS 文件到最终 MP4 | 约 46 分钟 |
| 最终文件 | `jLV1OL1Y8uA_run/full_dub_v5/jLV1OL1Y8uA_zh_dub_v5_1080p.mp4` |

三人物精细拆分后的 v6 增量案例：

| 指标 | 实际结果 |
|---|---:|
| 原槽 / 三人物台词 | 1,306 / 1,362 |
| 增量替换 | 55 个槽、111 条台词 |
| 人物 | 胡安 75；塞尔吉奥 183；阿德里安 1,104 |
| MiniMax 增量调用 | 6 次、3,065 计费字符、按量等价约 ¥1.0728 |
| 区块复用 | 375 个复用；62 个局部重编码 |
| 成片时长 | 5,284.723 秒，比 v5 增加约 7.11 秒 |
| 端到端墙钟时间 | 974.344 秒（约 16 分 14 秒，包含一次方案校正与完整验收） |
| 最终文件 | `jLV1OL1Y8uA_run/full_dub_v6_speaker_patch/jLV1OL1Y8uA_zh_dub_v6_speakers_1080p.mp4` |
