...
4. 格式探测 QA 通过后，按冻结的源语言、原声标记、真实分辨率、编码兼容性、帧率和码率规则自动选择画面与音频格式，记录 `qa/media_format_selection_vN.json` 后直接下载，不请求格式组合确认。只在章节翻译稿、角色音色、最终观看和可选外部交付登录这些真正需要人工判断或账户操作的关卡暂停。
