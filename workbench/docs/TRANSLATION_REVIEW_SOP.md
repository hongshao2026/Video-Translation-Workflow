...
如果冻结的大模型 Provider 暂不可用、额度不足或任务中断，必须保存进度并暂停/续跑。不得静默降级或切换 Provider。只有用户明确要求切换“额度优先模式”并重新生成 `workflow_lock.json` 后，才允许使用本地候选稿；双 Agent 全文审核仍不能取消。
...
遵守 `EVENT_DRIVEN_EXECUTION_SOP.md`。T/A/B 使用冻结职责包，不继承主控整个下载/排错历史；必要规范、全片上下文与全文覆盖不得省略。源文/审核包由确定性程序投影并绑定原文件哈希，逐词时间戳等证据按需查阅。T 不接收历史中文。A/B 输出问题、建议和完整覆盖证明，不重复生成未修改全文。主控等待完成事件，不主动查询进度。机械回归与章节阅读稿由固定程序派生，程序通过不冒充语义审核。
...
- 发布材料的纯展示层修改不需要重新启动 T/A/B 或使正式翻译失效；但必须创建新的 `deliverables/<video_id>_发布材料_vN.txt` 和 `qa/publication_materials_vN.json`，记录输入、输出和哈希。若修改反向影响字幕或 TTS 文本，则仍按第 8 节重新进入翻译审核。
...
冻结 Provider 额度不足时保持当前状态并等待恢复，不能把剩余片段交给其他模型后混入同一候选版本。若用户明确切换模式，必须新建候选版本、记录不同生成来源，并重新运行完整 A/B 审核。
